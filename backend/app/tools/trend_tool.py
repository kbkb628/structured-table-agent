...
def trend_analysis(
    ...
    sql = f"""
        SELECT "{group_by}", "{result_field}"
        FROM (
            SELECT
                CAST("{group_by}" AS VARCHAR) AS "{group_by}",
                TRY_CAST("{group_by}" AS DATE) AS "_sort_date",
                {aggregation}("{metric_column}") AS "{result_field}"
            FROM read_csv_auto(?)
            GROUP BY 1, 2
        ) grouped_trend
        ORDER BY "_sort_date" {sort_order.upper()}, "{group_by}" {sort_order.upper()}
        LIMIT ?
    """
    ...
    return ToolResponse(
        ...
        tool_name="trend_analysis",
        data={"rows": rows},
        summary="aggregate rows over the requested time dimension in chronological order",
        error=None,
        metadata={
