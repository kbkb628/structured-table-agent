...
def test_demo_script_mentions_project_status_summary_output():
    ...
    assert "project_status_latest_task_checkpoint_pending_metric_count" in script
    assert "project_status_latest_task_checkpoint_finding_count" in script
...
def test_demo_script_refreshes_project_status_after_demo_runs():
    ...
    assert "$projectStatus = Invoke-RestMethod" not in script
